...
const AddProfileModal = ({ isOpen, onClose, mode, initialData, onSave }) => {
  ...
  // 2. SYNC DATA (Includes mapping snake_case from DB to camelCase for Form)
  useEffect(() => {
  ...
  const handleFinalSave = async () => {
    // Basic validation to prevent saving people aged 31+
    if (parseInt(formData.age) >= 31) {
    ...
    try {
      ...
      onSave(); // Refreshes the list in ProfilesView
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-[#0D2440]/30 dark:bg-slate-900/80 backdrop-blur-sm" onClick={onClose} />
        ...
        <div className="px-8 py-5 flex justify-between items-center border-b border-gray-50 dark:border-slate-800 shrink-0">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${
              isAdd ? 'bg-emerald-50 text-emerald-600' : isEdit ? 'bg-blue-50 text-blue-600' : 'bg-gray-50 text-gray-600'
            }`}>
        ...
        <div className="p-8 overflow-y-auto">
          <div className="flex flex-col md:flex-row gap-8 items-start">
            ...
            <div className="flex flex-col items-center gap-4 shrink-0 w-full md:w-auto">
              ...
              <div onClick={triggerFileInput} className={`w-28 h-28 rounded-2xl overflow-hidden border-4 border-gray-50 dark:border-slate-800 shadow-lg bg-gray-100 dark:bg-slate-800 flex items-center justify-center group relative transition-all ${!isView ? 'cursor-pointer hover:border-blue-400/50' : ''}`}>
                {formData.image ? <img src={formData.image} alt="Profile" className="w-full h-full object-cover" /> : <User size={32} className="text-gray-300" />}
                {!isView && (
            ...
            <div className="flex-1 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                  ...
                  {/* BIRTHDATE FIELD WITH AUTO-AGE CALCULATION */}
                  <EditField 
                    label="Birthdate" 
                    type="date" 
