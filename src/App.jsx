...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col overflow-hidden">
      {/* HEADER: No container, purely aligned to the edges of the dashboard below */}
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* SIDEBAR: Pinned to left, aligned with header title */}
        <nav className={`pl-10 pr-4 py-2 space-y-2 transition-all duration-300 shrink-0 ${isSidebarShrinked ? 'w-24' : 'w-72'}`}>
          {[
            { id: 'dashboard', label: 'Dashboard', icon: Home },
            { id: 'youth', label: 'SK Profiles', icon: Users },
            { id: 'map', label: 'Brgy Map', icon: Map },
            ...
            <button 
              ...
              className={`w-full flex items-center gap-4 p-3 rounded-xl transition-all ${
                activeMenu === item.id ? 'bg-[#0D2440] text-white shadow-lg' : 'text-[#7BA4D0] hover:bg-[#e7f0fa]'
              }`}
            >
              <item.icon className="w-5 h-5 shrink-0" />
              {!isSidebarShrinked && <span className="font-semibold text-sm">{item.label}</span>}
            </button>
          ))}
        </nav>

        {/* MAIN AREA: No padding on the left to align with Sidebar */}
        <main className="flex-1 pr-10 pb-10 overflow-y-auto">
          {activeMenu === 'dashboard' && <DashboardView />}
          {activeMenu !== 'dashboard' && (
             <div className="bg-white rounded-2xl shadow-sm border border-[#7BA4D0]/20 p-12 text-center h-full">
